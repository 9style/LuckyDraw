...
- Produces:
  ...
  - `radarPoints(cx, cy, radius, values, max): Array<{x, y}>` —— **纯函数**，可测
  - `drawStarfield(ctx, w, h, rng): void`
  - `drawRadar(ctx, cx, cy, radius, values, labels, palette): void`
...
> `drawRadar` 的 `labels` 传的是**已翻译的维度名**（`摸鱼指数` 等），不是 key —— 由 `poster.js` 负责映射。
...
- Produces:
  ...
  - `buildVars(reading, cfg): { title, subtitle, name, dept, identity, scoreRow, fortuneLevel, fortuneVerse, fortuneYi, fortuneJi, scanHint, disclaimer }`
...
/**
 * 四维数值行的拼装（`摸鱼指数 73  ·  升值指数 78  ·  …`）。
 *
 * ⚠️ **执行期订正**：本计划原稿把这一行直接写在 `buildVars` 里（`dims.map(...).join(...)`），
 * 而 Task 8 之后的容量守卫又手抄了一份一模一样的拼接去量宽度 —— 两处一旦漂移
 *（改分隔符、改顺序、改兜底值），守卫就会对着一个海报上根本不存在的字符串算出「装得下」，
 * 而真正的海报上那行字正横穿画布。抽成导出函数后，实现与守卫调用的是同一个东西。
 *
 * 这一行还是全项目唯一**三重失守**的文案：标签来自 `scoring.json`，而 lint-copy.js
 * 只扫 `copy/*.json`、`settings.json`、`.vue` 与非测试 `.js` —— `scoring.json` 整个文件
 * 不在扫描面内，所以 `dimensionLabels` 既没有禁用词检查、也没有 60 字上限检查。
 */
...
export function renderPoster({ reading, cfg, canvas, qrImage }) {
  ...
  ctx.lineWidth = 2
  ctx.stroke()
  // ⚠️ **执行期订正**：本计划原稿这里是写死 `idBox.font` 直绘的。但标签文案由运营
  // 直接改 copy/identity.json，那边唯一的自动守卫只是 lint-rules 的「单条 ≤ 60 字」——
  // 60 字的标签在 36px 下要 2160px，写死字号就会顶穿烫金边框、横着画到画布上。
  // 实际交付的版本与姓名同一套降档，minFont 是配置里早就写明的下限。
  ...
  // 即按声明字号直绘、从不读 box.maxWidth —— 于是 poster.json 里的 `maxWidth: 620`
  // 对着一行最宽能画到 740px 的文案，形同虚设。四维全三位数是可达的
  //（taurus(90) + gt5(+8) + tech(+6) = 104，被 scoring.js 夹到 100）。
  // 下面是实际交付的版本：与 name / identity 同一套测量驱动降档。
...
const dataURL = ref('')
// ⚠️ **执行期订正**：本计划原稿没有这个 ref，下载后缀写死成 .png（见下方 download）。
// 后缀必须跟着 renderPoster 返回的 format 走：超过 1.2MB 时它会改用 JPEG。
...
function download() {
  ...
  // ⚠️ **执行期订正**：本计划原稿把后缀写死成 `.png`。但 renderPoster 在 PNG 超过
  // 1.2MB 时会改用 JPEG（深色渐变 + 星点压不动），而下载行为只看后缀 ——
  // 写死 .png 会得到一个「叫 png、内容却是 JPEG 字节」的文件，系统按 PNG 解码失败。
  // 后缀必须跟着 format 走。
 ...
 *
 * ⚠️ **执行期订正两处**：
 *  1. 原稿只有 12 条极端输入，没有基线条。但栅格里的缩略图最小只有 220px 宽，
 *     16px 的免责声明、二维码静默区、烫金边框的余量在那个尺寸下都判断不了；
 *     而 index.html 的 user-scalable=no 让「把浏览器放大」这条退路在手机上根本不成立。
 *     本页是本功能唯一的视觉验收工具，至少要有一张能按真实像素看的。
 *  2. 最后一条原稿写的是 `level: ''`，而下面读取的字段是 `c.fortuneLevel` ——
 *     键名对不上就永远读不到，这一条会悄悄退化成「和基线一样」，
 *     而它声称测的是空值兜底。
 */
const CASES = [
...
⚠️ **执行期订正（重要）**：本计划原稿这里教的是「顶部静态 import + 模板 `v-if="debugPoster"`」，
并断言「`import.meta.env.DEV` 在生产构建里是常量 false，整个分支会被摇掉，DebugPoster 不会进产物」。
**那条断言是错的，实测推翻了它。** `<script setup>` 会把模板里引用的绑定包一层 `unref()`，
条件于是编译成 `unref(false)`；`unref` 是运行时函数调用，压缩器折不掉这个三元，组件的引用
就一直挂着 —— 整个 DebugPoster（用例字符串 + 样式）照样进产物。

实际生效的写法是**模块级条件三元 + `defineAsyncComponent`**：
...
这里 `import.meta.env.DEV` 在生产构建被替换成常量 `false` 后，三元整条被折掉，
动态 import 根本不进依赖图，连它的 chunk 与样式都不会产出。

**验证方式**：`npm run build` 后在 `dist/assets/*.js` 里搜 DebugPoster 的用例字符串
（如 `欧阳司马诸葛上官`）。搜不到才算接对 —— 这个坑不会以任何其它方式报出来：
页面一切正常，只是产物白白大了一截。

- [ ] **Step 3: Verify in the browser**

启动 `npm run dev`，访问 `http://localhost:5174/?debug=poster`。
Expected: 13 条用例全部渲染（1 张 1:1 原尺寸 + 12 张缩略图），无异常；逐条检查下面这份清单并记录结果：
